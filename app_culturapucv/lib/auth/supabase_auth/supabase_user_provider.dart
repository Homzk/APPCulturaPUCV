...
class AppCulturapucvSupabaseUser extends BaseAuthUser {
  AppCulturapucvSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> appCulturapucvSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AppCulturapucvSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
