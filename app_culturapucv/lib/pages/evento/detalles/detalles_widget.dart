  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<EventsRow>>(
      ...
      builder: (context, snapshot) {
        ...
        List<EventsRow> detallesEventsRowList = snapshot.data!;

        final detallesEventsRow = detallesEventsRowList.isNotEmpty
            ? detallesEventsRowList.first
            : null;
                          ...
                          return Material(
                            ...
                            child: Container(
                              ...
                              child: FFButtonWidget(
                                onPressed: () async {
                                  _model.inscripRealizadas =
                                      await CultupucvGroup
                                          .contarReservasUsuarioCall
                                          .call(
                                    usuarioId: containerUsersRow?.id,
                                  );

                                  _model.disponibilidad =
                                      await VerificarDisponibilidadCall.call(
                                    eventoId: widget!.id,
                                  );

                                  if ((_model.disponibilidad?.succeeded ??
                                      true)) {
                                    if ((int disponibilidad) {
                                      return disponibilidad >= 1;
                                    }((_model.disponibilidad?.jsonBody ??
                                        ''))) {
                                      if ((_model.inscripRealizadas?.jsonBody ??
                                              '') ==
                                          0) {
                                        context.pushNamed(
                                          'inscripcion',
                                      ...
                                      } else if (((_model.inscripRealizadas
                                                      ?.jsonBody ??
                                                  '') >
                                              0) &&
                                          !containerUsersRow!.isComplete!) {
                                        await showDialog(
                                          context: context,
                                          builder: (alertDialogContext) {
                                            return AlertDialog(
                                              title: Text('hola'),
                                              content: Text('completar perfil'),
                                              actions: [
                                                TextButton(
                                                  onPressed: () =>
                                                      Navigator.pop(
                                    ...
                                    await showDialog(
                                      ...
                                      builder: (alertDialogContext) {
                                        return AlertDialog(
                                          content: Text('Error al reservar :('),
                                          actions: [
                                            TextButton(
                                              onPressed: () => Navigator.pop(
                                                  alertDialogContext),
